/**
 * Sound synthesis using HTML5 Web Audio API.
 * Provides tactile audio feedback for task completions, quick notes, coin drops, and reward chimes.
 */
...
/**
 * Cheerful crisp double chime for completing a task
 */
...
export function playTaskUncheck() {
  if (!soundEnabled) return;
  const ctx = getAudioContext();
  if (!ctx) return;
...
/**
 * Legacy pencil stroke alias pointing to playTaskDone
 */
export function playPencilStrike() {
  playTaskDone();
}

/**
 * Soft popping sound for creating quick notes or adding items
 */
...
/**
 * Coin clink sound for personal finance jar updates & piggy bank allocations
 */
